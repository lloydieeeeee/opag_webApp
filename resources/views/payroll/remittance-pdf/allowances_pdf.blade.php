...
<head>
    <meta charset="utf-8">
    <title>Allowances Remittance - {{ $period->period_label }}</title>
    <style>
        ...
        .data-table th { text-align: center; font-weight: normal; font-size: 9px; text-transform: uppercase; background-color: #fff; font-family: "Times New Roman", Times, serif;}
        .data-table td { font-size: 9px; font-family: "Times New Roman", Times, serif;}
...
<body>
    ...
    <div class="report-title">ALLOWANCES REPORT (PERA, RATA, TA, OTHER)</div>
    <div class="report-subtitle">For the period covered {{ strtoupper($period->period_label) }}</div>

    @php
        $filtered = $records->filter(fn($r) => ($r->allowance_pera??0)>0 || ($r->allowance_rata??0)>0 || ($r->allowance_ta??0)>0 || ($r->allowance_other??0)>0)->values();
        $totPera = $filtered->sum('allowance_pera');
        $totRata = $filtered->sum('allowance_rata');
        $totTa = $filtered->sum('allowance_ta');
        $totOther = $filtered->sum('allowance_other');
        ...
        <tbody>
            @foreach($filtered as $index => $r)
                <tr>
                    ...
                    <td class="text-right">{{ $fmt($r->allowance_pera) }}</td>
                    <td class="text-right">{{ $fmt($r->allowance_rata) }}</td>
                    <td class="text-right">{{ $fmt($r->allowance_ta) }}</td>
                    <td class="text-right">{{ $fmt($r->allowance_other) }}</td>
                </tr>
            @endforeach
            <tr>
                <td colspan="6" class="text-right font-bold">TOTAL</td>
                <td class="text-right font-bold">{{ $totPera > 0 ? number_format($totPera, 2) : '-' }}</td>
                <td class="text-right font-bold">{{ $totRata > 0 ? number_format($totRata, 2) : '-' }}</td>
                <td class="text-right font-bold">{{ $totTa > 0 ? number_format($totTa, 2) : '-' }}</td>
                <td class="text-right font-bold">{{ $totOther > 0 ? number_format($totOther, 2) : '-' }}</td>
            </tr>
        </tbody>
    </table>
