...
<head>
    <meta charset="utf-8">
    <title>Withholding Tax Remittance - {{ $period->period_label }}</title>
    <style>
...
<body>
    ...
    <div class="report-title">STATEMENT OF REMITTANCE FOR WITHHOLDING TAX</div>
    <div class="report-subtitle">For the period covered {{ strtoupper($period->period_label) }}</div>

    @php
        $filtered = $records->filter(fn($r) => ($r->withholding_tax??0)>0)->values();
        $totTax = $filtered->sum('withholding_tax');
        ...
        <tbody>
            @foreach($filtered as $index => $r)
                <tr>
                    ...
                    <td class="text-center"></td> {{-- Blank TIN per request --}}
                    <td class="text-right">{{ $fmt($r->withholding_tax) }}</td>
                </tr>
            @endforeach
            <tr>
                <td colspan="7" class="text-right font-bold">TOTAL</td>
                <td class="text-right font-bold">{{ number_format($totTax, 2) }}</td>
            </tr>
        </tbody>
    </table>
