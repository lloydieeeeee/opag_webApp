...
<head>
    <meta charset="utf-8">
    <title>DBP Remittance - {{ $period->period_label }}</title>
    <style>
        ...
        .data-table th { text-align: center; font-weight: normal; font-size: 10px; text-transform: uppercase; background-color: #fff; }
...
<body>
    ...
    <div class="report-title">DEVELOPMENT BANK OF THE PHILIPPINES (DBP) LOAN AMORTIZATION REMITTANCE</div>
    <div class="report-subtitle">For the period covered {{ strtoupper($period->period_label) }}</div>

    @php
        $filtered = $records->filter(fn($r) => ($r->loan_dbp??0)>0)->values();
        $totDbp = $filtered->sum('loan_dbp');
        ...
        <tbody>
            @foreach($filtered as $index => $r)
                <tr>
                    ...
                    <td class="text-right">{{ $fmt($r->loan_dbp) }}</td>
                </tr>
            @endforeach
            <tr>
                <td colspan="6" class="text-right font-bold">TOTAL</td>
                <td class="text-right font-bold">{{ number_format($totDbp, 2) }}</td>
            </tr>
        </tbody>
    </table>
