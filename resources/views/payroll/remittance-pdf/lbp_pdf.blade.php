...
<head>
    <meta charset="utf-8">
    <title>LBP Remittance - {{ $period->period_label }}</title>
    <style>
...
<body>
    ...
    <div class="report-title">LAND BANK OF THE PHILIPPINES (LBP) LOAN AMORTIZATION REMITTANCE</div>
    <div class="report-subtitle">For the period covered {{ strtoupper($period->period_label) }}</div>

    @php
        $filtered = $records->filter(fn($r) => ($r->loan_lbp??0)>0)->values();
        $totLbp = $filtered->sum('loan_lbp');
        ...
        <tbody>
            @foreach($filtered as $index => $r)
                <tr>
                    ...
                    <td class="text-right">{{ $fmt($r->loan_lbp) }}</td>
                </tr>
            @endforeach
            <tr>
                <td colspan="6" class="text-right font-bold">TOTAL</td>
                <td class="text-right font-bold">{{ number_format($totLbp, 2) }}</td>
            </tr>
        </tbody>
    </table>
