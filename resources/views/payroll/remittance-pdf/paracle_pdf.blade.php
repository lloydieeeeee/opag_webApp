...
<head>
    <meta charset="utf-8">
    <title>Paracale Remittance - {{ $period->period_label }}</title>
    <style>
...
<body>
    ...
    <div class="report-title">RURAL BANK OF PARACALE LOAN AMORTIZATION REMITTANCE</div>
    <div class="report-subtitle">For the period covered {{ strtoupper($period->period_label) }}</div>

    @php
        $filtered = $records->filter(fn($r) => ($r->loan_paracle??0)>0)->values();
        $totPar = $filtered->sum('loan_paracle');
        ...
        <tbody>
            @foreach($filtered as $index => $r)
                <tr>
                    ...
                    <td class="text-right">{{ $fmt($r->loan_paracle) }}</td>
                </tr>
            @endforeach
            <tr>
                <td colspan="6" class="text-right font-bold">TOTAL</td>
                <td class="text-right font-bold">{{ $totPar > 0 ? number_format($totPar, 2) : '-' }}</td>
            </tr>
        </tbody>
    </table>
