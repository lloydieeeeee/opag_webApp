    ...
    <div class="mgmt-card">
        ...
        <div class="ch">
            <div>
                ...
                <p class="cd">Manage commutation options for leave applications</p>
            </div>
            <div style="display:flex;align-items:center;gap:10px;">
                <div class="sw">
                    ...
                    <input type="text" id="si" placeholder="Search commutation..." oninput="filterRows()">
                </div>
        ...
        <div class="tsa">
            <table class="dt">
                ...
                <tbody id="dBody">
                    ...
                    <tr class="dr" data-id="{{ $opt->id }}" data-name="{{ strtolower($opt->label) }}">
                    ...
                    @empty
                    <tr>
                        <td colspan="3" style="padding:52px;text-align:center;color:#9ca3af;font-size:13px;">No commutation options found.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
...
<div id="fo" class="ov" onclick="if(event.target===this)closeForm()">
    <div class="mc">
        ...
        <p  class="ms" id="mS">Enter the commutation label below.</p>
        <div class="fr">
            <label class="fl">Label *</label>
            <input class="fi" id="fN" type="text" placeholder="e.g. Requested">
        </div>
        <div class="mf">
...
<div id="do" class="ov" onclick="if(event.target===this)closeDel()">
    <div class="mc" style="width:380px;">
        ...
        <p style="font-size:13px;color:#6b7280;margin:0 0 24px;">
            You are about to delete <strong id="dN"></strong>. This may affect existing leave applications.
        </p>
...
const BASE  = '{{ route("settings.commutation") }}';
let eid = null, did = null;

function filterRows() {
    const q = document.getElementById('si').value.toLowerCase();
    document.querySelectorAll('.dr').forEach(r => r.style.display = r.dataset.name.includes(q) ? '' : 'none');
}

function openAdd() {
    eid = null;
    document.getElementById('mT').textContent = 'Add Commutation Option';
    document.getElementById('mS').textContent = 'Enter the commutation label below.';
    document.getElementById('bS').textContent = 'Add Option';
...
function openEdit(id, label) {
    eid = id;
    document.getElementById('mT').textContent = 'Edit Commutation Option';
    document.getElementById('mS').textContent = 'Update the commutation label.';
    document.getElementById('bS').textContent = 'Save Changes';
    document.getElementById('fN').value = label;
    document.getElementById('fo').classList.add('show');
    setTimeout(() => document.getElementById('fN').focus(), 150);
}
...
function saveForm() {
    const label = document.getElementById('fN').value.trim();
    ...
    fetch(eid ? `${BASE}/${eid}` : BASE, {
        ...
        b.disabled = false;
        b.textContent = eid ? 'Save Changes' : 'Add Option';
...
function openDel(id, label) {
    did = id;
    document.getElementById('dN').textContent = label;
